...
Do not use `.map` on optionals to unwrap them. `.map` should only be used for its semantic purpose: transforming elements of a collection. For optionals, use `guard let`, `if let`, or `??` instead.
